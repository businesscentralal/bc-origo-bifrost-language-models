...
### Fixed (2026-09-25) - install and upgrade skip unguarded table access

- **`Copilot Install ori.RegisterSecrets`**, called from `OnInstallAppPerCompany` and `Copilot Upgrade ori.OnUpgradePerCompany`, no longer reads `Bifrost Language Model ori` unless `ReadPermission` succeeds. `LangModel Secrets ori.RegisterAll` `FindSet()`s that table and then calls Foundation `Secret Store ori.Register`, which writes `App Secret ori` under inherent permissions. A missing read skips registration with telemetry `ORI-BIF-0423` (Warning, `tableId` 10035335, `deniedPermission` Read). The setup page still calls `RegisterAll` directly, so an administrator opening Language Models setup still registers secrets.
- **`ClaimChatProvider`** now requires `InsertPermission` and `ModifyPermission` on `Setup ori`, not `WritePermission`. `GetRecordOnce` inserts the singleton when it is missing and the claim then `Modify()`s it. `WritePermission` is true when only Delete is granted, so that check could still let the data action throw.

### Fixed (2026-09-25) - install claim skips when Setup ori is not permitted (core#122)

- **`Copilot Install ori.ClaimChatProvider`**, called from `OnInstallAppPerCompany`, no longer reads Foundation `Setup ori` unless `ReadPermission`, `InsertPermission` and `ModifyPermission` all succeed. Publishing Foundation re-ran this install in a context with no TableData Read on `Setup ori` (id 10077901), and `GetRecordOnce` failed the deploy (OrigoSoftwareSolutions/bc-origo-bifrost-core#122). The claim is skipped instead; an existing chat provider is left unchanged.
- **Admin signal**: each skip branch logs telemetry event `ORI-BIF-0422` (Warning, `tableId` 10077901 and `deniedPermission` Read, Insert or Modify). `Chat Provider Type` then stays None, and nothing retries the claim automatically, so an administrator has to set it.
- **Test**: `Copilot Install Tests` (96018) `ClaimChatProvider_WithoutSetupPermission_SkipsWithoutError` — restrictive permissions (`Library - Lower Permissions`, O365 Basic + `BIFROST LLM ori`) skip without error; `ClaimChatProvider_WhenNone_ClaimsLanguageModels` covers the permitted path. `RegisterSecrets_WithoutLanguageModelRead_SkipsWithoutError` uses test permission set `LM Install Exec Tst` (96019), which can run the install codeunit but cannot read `Bifrost Language Model ori`; `RegisterSecrets_WithReadPermission_RegistersSecrets` covers the permitted path.
