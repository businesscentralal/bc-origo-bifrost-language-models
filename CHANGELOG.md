...
- **Test**: `Copilot Install Tests` (96018) `RegisterSecrets_WithoutLanguageModelRead_SkipsWithoutError` uses test permission set `LM Install Exec Tst` (96019), which can run the install codeunit but cannot read `Bifrost Language Model ori`; `RegisterSecrets_WithReadPermission_RegistersSecrets` covers the permitted path.
